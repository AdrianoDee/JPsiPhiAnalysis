...
DiMuonVtxReProducer::DiMuonVtxReProducer(const edm::Handle<reco::VertexCollection> &handle, const edm::Event &iEvent)
{
    ...
    if (!foundTracks || !foundBeamSpot) {
        edm::LogWarning("DiMuonVtxReProducer_MissingParentage") <<
...
void
DiMuonVtxReProducer::configure(const edm::ParameterSet &iConfig)
{
    config_ = iConfig;
...
DiMuonVtxReProducer::makeVertices(const reco::TrackCollection &tracks,
...
std::vector<TransientVertex>
DiMuonVtxReProducer::makeVertices(const std::vector<reco::TransientTrack> &tracks,
                               const reco::BeamSpot &bs,
                               const edm::EventSetup &iSetup) const
{
    edm::ESHandle<TransientTrackBuilder> theB;
