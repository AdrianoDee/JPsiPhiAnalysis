...
// muons_(consumes<edm::View<pat::Muon>>(iConfig.getParameter<edm::InputTag>("traks"))),
...
//
//
//   // MuMu candidates only from traks
//   for (size_t j = 0; j < traks->size(); j++){
//     auto tNeg = (*traks)[i];
//     // both must pass low quality
//     if(tNeg.charge()>=0.0) continue;
//     if(!lowerPuritySelection_(tNeg)) continue;
//
//     for (size_t j = 0; j < traks->size(); j++){
//
//       auto tPos = (*traks)[j];
//       if(tPos.charge()<=0.0) continue;
//       if(i == j) continue;
//       // both must pass low quality
//       if(!lowerPuritySelection_(tPos)) continue;
//       //std::cout << "Second muon quality flag" << std::endl;
//       // one must pass tight quality
//       if (!(higherPuritySelection_(tNeg) || higherPuritySelection_(tPos))) continue;
//
//       // ---- fit vertex using Tracker tracks (if they have tracks) ----
//       if (!(tNeg.track().isNonnull() && tPos.track().isNonnull())) continue;
//
//       pat::CompositeCandidate mumucand;
//       vector<TransientVertex> pvs;
//
//       mumucand.addDaughter(tNeg,"muonN");
//       mumucand.addDaughter(tPos,"muonP");
//
//       // ---- define and set candidate's 4momentum  ----
//       LorentzVector mumu = tNeg.p4() + tPos.p4();
//       TLorentzVector mu1, mu2,mumuP4;
//       mu1.SetXYZM(tNeg.track()->px(),tNeg.track()->py(),tNeg.track()->pz(),muon_mass);
//       mu2.SetXYZM(tPos.track()->px(),tPos.track()->py(),tPos.track()->pz(),muon_mass);
//       // LorentzVector mumu;
//
//       mumuP4=mu1+mu2;
//       mumucand.setP4(mumu);
//       mumucand.setCharge(tNeg.charge()+tPos.charge());
//
//       if(!dimuonSelection_(mumucand)) continue;
//
//       float deltaRMuMu = reco::deltaR2(tNeg.eta(),tNeg.phi(),tPos.eta(),tPos.phi());
...
//       muon_ttks.push_back(theTTBuilder->build(tNeg.track()));  // pass the reco::Track, not  the reco::TrackRef (which can be transient)
//       muon_ttks.push_back(theTTBuilder->build(tPos.track())); // otherwise the vertex will have transient refs inside.
//
...
//                 // Primary vertex matched to the dimuon, now refit it removing the two traks
...
//                   const reco::Muon *rmu1 = dynamic_cast<const reco::Muon *>(tNeg.originalObject());
//                   const reco::Muon *rmu2 = dynamic_cast<const reco::Muon *>(tPos.originalObject());
//                   // check that traks are truly from reco::Muons (and not, e.g., from PF Muons)
...
//           // ---- MC Truth, if enabled ----
//           if (addMCTruth_) {
//             reco::GenParticleRef genMu1 = tNeg.genParticleRef();
//             reco::GenParticleRef genMu2 = tPos.genParticleRef();
