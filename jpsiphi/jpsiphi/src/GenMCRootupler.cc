...
//
//   if (isMC_ || OnlyGen_) {
//      // std::cout << "GenMCRootupler::GenMCRootupler: Dimuon id " << pdgid_ << std::endl;
...
//       }  // if ( p_id
//     } // for (size
//     // if ( dimuon_pdgId ) std::cout << "GenMCRootupler: found the given decay " << run << "," << event << std::endl; // sanity check
//   }  // end if isMC
//
...
//           if (OnlyBest_) break;
//           else {
//             gen_tree->Fill();   // be aware, we are storing all combinations
//             already_stored = true;
//           }
//         }
//       }
//     } //..else {
//       //std::cout << "GenMCRootupler: (" << run << "," << event << ") -> ";
//
//   }  // !OnlyGen_
//
//   if ( !already_stored ) {  // we have to make sure, we are not double storing an combination
//     if ( !isMC_ ) {
//       if ( ndimuon > 0 ) gen_tree->Fill();   // if not MC filter out
//     } else gen_tree->Fill();
//   }
// }
//
